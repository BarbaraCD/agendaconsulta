...
export const Appointments: React.FC = () => {
  ...
  const [selectedDoctor, setSelectedDoctor] = useState<DoctorsProps | undefined>(undefined);
  const [patients, setPatients] = useState<PatientsProps[]>([])
  const [selectedPatient, setSelectedPatient] = useState<PatientsProps | undefined>(undefined);
  ...
  const handleDoctorSelect = (value: number) => {
    ...
    setSelectedDoctor(selectedDoc?.id);
  };

  const handlePatientSelect = (value: number) => {
    const selectedPat = patients.find((pat) => pat.id === value);
    setSelectedPatient(selectedPat?.id);
  };
