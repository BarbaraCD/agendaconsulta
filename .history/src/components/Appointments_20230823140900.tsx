...
export const CreateAppointments: React.FC = () => {
  ...
  async function fetchInfos() {
    if (!id) return
    try {
      const responseDoc = await getDoctorById()
      const responsePat = await getPatientById()
      const responseAp = await getAppointmentById()
      reset(responseDoc as unknown as DoctorSchemaType)
      reset(responsePat as unknown as PatientSchemaType)
