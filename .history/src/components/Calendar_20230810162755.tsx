...
import { CalendarStyled, ApFooter, StyledLink, StyledModal, StyledLink2 } from '../styles/Appointments';
...
export const AppointmentsCalendar = () => {
  ...
  const onClickEdit = async (appointment: AppointmentsProps) => {
    try {
      ...
      setSelectedAppointment(selectedAP.id);
    } catch (error) {
      ...
      >
        {selectedAppointment && (
          <div>
            ...
            <div>
              <button onClick={() => onClickEdit(selectedAppointment)}><StyledLink2 to={`/appointments/${selectedAppointment.id}`}><EditOutlined /></StyledLink2></button>
